...
class MyWindow(QtWidgets.QWidget):
   ...
   def tra(self,data):
    ...
    #data = pd.read_csv('data.csv')
      ...
      adm=keras.optimizers.Adam(lr=0.02, beta_1=0.9, beta_2=0.999, epsilon=None, decay=0.0, amsgrad=False)
      classifier.compile(loss='categorical_crossentropy',optimizer=adm,metrics=['accuracy'])
      #classifier.summary()
      classifier.fit(X_train,y_train,epochs=300,batch_size=4,validation_data=(X_train,y_train),verbose=1)
      z1=classifier.evaluate(X_train,y_train,verbose=0)
      z2=classifier.evaluate(X_test,y_test,verbose=0) 
      print("\n") 	 
      print(" Model Train Accuracy = ",z1[1],end=' ')
      print("Model Test Accuracy = ",z2[1])
      del classifier
def stylesheet(self):
       return """
...
main.setWindowTitle("CSV Handler - Group 6")
main.show()
